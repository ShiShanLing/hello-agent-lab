...
**发布到服务器时，必须把对应源码和本次构建生成的 `dist/` 提交并推到 GitHub，作为同一套流程一次做完。** 用户说「提交」就包含 `git push`，禁止只 commit 完再停下来问。不要提交 `.env`、密钥、数据库、上传文件、缓存或日志。三个前端的 `dist/` 不得加入 `.gitignore`；正式构建后应检查并提交对应构建产物，确保 GitHub 与生产版本可追溯。

## 分支与 GitHub 同步长期规则

以下规则适用于 Codex、Cursor 和其他在本仓库工作的开发工具，属于必须长期遵守的项目约束：

1. **禁止自动切换分支。** 开始工作后必须留在当前分支；不得因为修复、发布、同步、解决冲突或工具默认行为自行执行 `git switch`、`git checkout`、创建新分支、进入 detached HEAD，或改到其他 worktree。任何会改变当前分支的操作，都必须先向用户说明目标分支和原因，得到明确同意后才能执行。
2. **打包或发布必须先同步 GitHub。** 用户要求打包、部署或发布时，必须按“检查当前分支与改动 → 测试/构建 → 检查并提交本次相关源码和对应 `dist/` → 推送当前分支到 GitHub → 确认远程包含该提交 → 发布服务器 → 线上验收”的顺序一次完成。`git push` 失败时不得继续发布；禁止出现生产代码领先于 GitHub 的情况。
3. **GitHub 改动描述必须使用中文。** Git 提交标题与正文、GitHub Pull Request 标题与说明、Release 标题与更新说明均使用清晰中文；确需保留的命令、文件名、标识符和代码术语除外。
4. 提交和发布前后都要再次检查当前分支，确认没有发生未经用户同意的分支变化，并在最终结果中报告当前分支、提交号、GitHub 推送结果和发布验收结果。
...
- 不要把不同前端的构建产物相互复制或合并。
- `frontend/dist/`、`todo-frontend/dist/`、`admin-frontend/dist/` 不得被 `.gitignore` 忽略；正式构建和发布时应将对应构建产物与源码一起提交、推送。
